...
export async function POST(req: NextRequest) {
  try {
    const { prompt, style_group_id, style_id } = await req.json()
    const taskRes = await fetch(`${BASE}/s2s/v1.0/task/text-to-image`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${KEY()}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({
        request_id: Date.now(),
        payload: {
          actions: [{ id: 0, params: { prompt, style_group_id, style_ids: [style_id] } }],
          output_ext: 'jpg',
        },
      }),
    })
    ...
    for (let i = 0; i < 60; i++) {
      const poll = await fetch(`${BASE}/s2s/v1.0/task/text-to-image?task_id=${encodeURIComponent(taskId)}`, {
